...
async function submitAkashGaikwadApplication() {
  ...
  formData.append("_subject", "🎓 NEW M.A. ADMISSION APPLICATION: Akash Gaikwad [XINRM-2026-AG772]");
  ...
  formData.append("attachment_passport_photo", photoBlob, "akash_gaikwad_passport_photo.png");

  const degreeBlob = new Blob([degreeBuf], { type: 'application/pdf' });
  formData.append("attachment_degree_certificate", degreeBlob, "bsc_degree_marksheet_sppu.pdf");

  const categoryBlob = new Blob([categoryBuf], { type: 'application/pdf' });
  formData.append("attachment_caste_certificate", categoryBlob, "category_caste_certificate_self_attested.pdf");

  console.log("Submitting official demo application for Akash Gaikwad to xinrmsocialcentre50@gmail.com...");
